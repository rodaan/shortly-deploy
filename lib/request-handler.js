  ...
  User.findOne({'username': username}).exec(function(err, user) { 
    if (err) {
      ...
      if (!user) {
        ...
        console.log('the 1st parameter is PSAWRD', password);
        console.log('the USER PSWORD-----------------', user.password);
  ...
  Link.findOne({ 'code': req.url.substring(1)}).exec(function(err, link) {
    ...
    if (!link) {
      ...
      Link.findById(link._id, function(err, link) {
        console.log('link after getting from database:', link);
        if (err) {
        ...
        link.save(function(err, link) {
          ...
          // console.log('dat link thooo;;;;;;;;;;;;;;;;;', link);
          // var thatlink = link.url;
          // res.redirect(thatlink);
